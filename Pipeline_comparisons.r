...
setwd("~/Documents/Thesis/NanoASV/Software_dev/Benchmark_Nygaard/")
...
load("NanoASV_Nygaard_dataset/NanoASV_output/map-ont-tree.out/Results/Rdata/NanoASV.rdata")
...
#MOCK.phy <- transform_sample_counts(MOCK.phy, function(x) x/sum(x))
...
rich_plot <- ggplot(data = alpha_tab, aes(x = ena_accession, y = richness)) +
  ...
  facet_wrap(~ Pipeline) +
  #geom_point(position = position_jitter(width = .20), alpha = 0.5, size = 5) + theme_bw() +
  #geom_boxplot(aes(colour = Refs), alpha=0.1, outlier.colour = NA) +
